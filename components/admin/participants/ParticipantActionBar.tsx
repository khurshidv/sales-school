...
import { PARTICIPANT_STATUS_CONFIG } from './ParticipantStatusBadge';
...
const STATUS_ACTIONS: Array<{ value: 'in_progress' | 'done' | 'hire' | 'skip' }> = [
  { value: 'in_progress' },
  { value: 'done' },
  { value: 'hire' },
  { value: 'skip' },
...
export function ParticipantActionBar({
  selectedIds,
  selectedPhones,
  onClear,
  onRefresh,
  ...
  function copyPhones() {
    navigator.clipboard.writeText(selectedPhones.join('\n')).then(
      () =>
        toast.push({
          tone: 'success',
          title: `${selectedPhones.length} номеров скопировано`,
        }),
    ...
    <div className="admin-action-bar">
      ...
      {STATUS_ACTIONS.map(a => (
        <button
          ...
          disabled={busy}
        >
          <UserCheck size={12} /> {PARTICIPANT_STATUS_CONFIG[a.value].label}
        </button>
      ))}
